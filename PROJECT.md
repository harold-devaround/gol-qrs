...
tests/
  ...
  segment-tool.test.ts       ← 11 tests (création, cancel, régression curseur fantôme, hasMoved, marqueur visuel p1)
  perpendicular-tool.test.ts ← 24 tests
  tab-router.test.ts         ← 19 tests (activation, visibilité, lazy-init, isolation erreurs)
  fabric-canvas-touch.test.ts← 68 tests (jsdom, tap/drag/pinch detection, hasMoved, mobile tap world coordinate regression, pinch ne perturbe pas l'état des outils, snap thresholds par modalité d'entrée, suppression du mouse:up synthétisé par Fabric après pinch)
  ...
  image-viewer.test.ts       ← 10 tests (jsdom, anti-XSS textContent, lightbox open/close, key handling)
  point-tool.test.ts         ← 6 tests (label collision avoidance, gap-filling, A1 fallback)
  TOTAL                      ← 504 tests (includes 78 jsdom)
```
...
```bash
npm test              # npx vitest run — lance les 504 tests
